...
__FLAME_GPU_FUNC__ int die(xmachine_memory_Boid* xmemory, xmachine_message_location_list* location_messages, xmachine_message_boid_dead_list* boid_dead_messages) {
	//Agent position vector
	//glm::vec3 agent_position = glm::vec3(xmemory->x, xmemory->y, xmemory->z);
	//glm::vec3 agent_velocity = glm::vec3(xmemory->fx, xmemory->fy, xmemory->fz);

	//Apply the velocity
	//agent_position += agent_velocity * 0;
	...
	//xmemory->y = agent_position.y;
	//xmemory->z = agent_position.z;
	int boid_id = xmemory->id;

	add_boid_dead_message(boid_dead_messages, boid_id);
