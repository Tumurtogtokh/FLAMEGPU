...
{
	add_location_message(location_messages, xmemory->id, xmemory->x, xmemory->y, xmemory->fx, xmemory->fy);
...
__FLAME_GPU_FUNC__ int inputdata(xmachine_memory_Boid* xmemory, xmachine_message_location_list* location_messages) 
{
	...
	glm::vec2 agent_velocity = glm::vec2(xmemory->fx, xmemory->fy);

	//Boids perceived centre
	glm::vec2 perceived_centre = glm::vec2(0.0f, 0.0f);
	int perceived_count = 0;

	//Boids global velocity matching
	glm::vec2 global_velocity = glm::vec2(0.0f, 0.0f);

	//Boids short range avoidance centre
	glm::vec2 collision_centre = glm::vec2(0.0f, 0.0f);
    ...
    while(location_message)
	{
		...
		glm::vec2 message_position = glm::vec2(location_message->x, location_message->y);
		...
		if (location_message->id != xmemory->id){
			...
			if (separation < (INTERACTION_RADIUS)){
				...
				glm::vec2 message_velocity = glm::vec2(location_message->fx, location_message->fy);
	...
	glm::vec2 velocity_change = glm::vec2(0.0f, 0.0f);


	//Rule 1) Steer towards perceived centre of flock (Cohesion)
	glm::vec2 steer_velocity = glm::vec2(0.0f, 0.0f);
	...
	glm::vec2 match_velocity = glm::vec2(0.0f, 0.0f);
